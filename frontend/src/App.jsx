...
function App() {
  ...
  useEffect(() => {
    ...
    map.current = new maplibregl.Map({
      ...
      center: [-79.383184, 43.653226], //Default location is Toronto
      zoom: 9,
    });
  }, []);
  ...
  async function fetchRoute(origin, destination) {
    ...
    const body = {
      locations: [
        { lon: origin.lng, lat: origin.lat, radius: 10 },
        { lon: destination.lng, lat: destination.lat, radius: 10 },
      ],
      costing: "auto",
    ...
    map.current.addLayer({
      ...
      source: "route",
      paint: {
        "line-color": routeColors[routeMode],
        "line-width": 4,
      },
    });
    ...
    <div style={{ position: "relative", width: "100vw", height: "100vh" }}>
      ...
      >
        <SearchBox
          key={`origin-${resetKey}`}
          label="Origin"
          onSelect={(p) => setPoints((prev) => [p, prev[1]].filter(Boolean))}
        />
        <SearchBox
          key={`destination-${resetKey}`}
          label="Destination"
          onSelect={(p) => setPoints((prev) => [prev[0], p].filter(Boolean))}
        />
        {routeInfo && (
          <div
            style={{
          ...
          >
            <span>
              🛣️ <strong>{routeInfo.distance} km</strong>
            </span>
            <span>
              ⏱️ <strong>{routeInfo.duration} min</strong>
            </span>
          </div>
        )}
      ...
      >
        {[
          { id: "normal", label: "🗺️ Normal", color: "#3b82f6" },
          { id: "newDriver", label: "🚗 New Driver", color: "#f59e0b" },
          { id: "scenic", label: "🌲 Scenic", color: "#22c55e" },
        ].map((mode) => (
          <label
            key={mode.id}
            style={{
              ...
              gap: "0.5rem",
              cursor: "pointer",
              fontWeight: routeMode === mode.id ? "bold" : "normal",
              color: routeMode === mode.id ? mode.color : "#374151",
            }}
          >
            <input
              type="radio"
